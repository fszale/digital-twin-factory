...
- super admins can access every deployment
- twin owners can access only assigned deployment data
- ownership is stored in deployment access records, not hardcoded in the app
...
- `baseline_value`
- `target_value`
- `hitl_notification_policy`

Implementation note:

- runtime deployment configuration is persisted in `deployment_configs`
- preferred model state is stored as `preferred_model_provider`, `preferred_model`, and `preferred_model_profile`
- fallback model state is stored separately for controlled failover

### `platform_user`

Human user mapped from Supabase Auth into platform roles.
...
Deployment-scoped access mapping for human users.
...
Runtime deployment configuration persisted for the control plane.
...
Synthesized summary of a conversation or run for reuse, review, or handoff.
...
Escalation record created when the real human behind the twin is needed.
...
Delivery record for dashboard, email, or Slack notifications tied to HITL or review events.
...
twin_deployment -> conversation_summary
